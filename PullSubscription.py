...
def pull_messages(subscription_name, max_messages=10, wait_time="3s"):
    ...
    :param wait_time: 메시지 fetch 대기 시간 (기본: "0.1s", 최대: "30s")
    :return: 수신된 메시지 리스트
    """
    url = f"{PUBSUB_ENDPOINT_URL}/v1/domains/{PUBSUB_DOMAIN_ID}/projects/{PUBSUB_PROJECT_ID}/subscriptions/{subscription_name}/pull"
    ...
    payload = {
        ...
        "waitTime": wait_time  # 문자열 형식으로 전달
    }
...
def ack_messages(subscription_name, ack_ids, retries=3):
    ...
    url = f"{PUBSUB_ENDPOINT_URL}/v1/domains/{PUBSUB_DOMAIN_ID}/projects/{PUBSUB_PROJECT_ID}/subscriptions/{subscription_name}/acknowledge"
...
def main():
    ...
    subscription_name = 'PullSubscription'  # 실제 Subscription 이름으로 변경
